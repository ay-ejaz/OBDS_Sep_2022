...
```{r}
set.seed(1)
vect <- rnorm(n = 1000, mean = 10, sd = 5)
...
```{r}
quant10 = quantile(vect, seq(0,1,0.1)) #go from probability 0 to 1 with steps of 0.1
quant10
```
...
```{r}
hist(vect, breaks = 20)
abline(v=mean(vect), col='red')
abline(v=median(vect), col='red', lty=2)
abline(v=mean(vect)-sd(vect), col='blue')
abline(v=mean(vect)+sd(vect), col='blue')
...
```{r}
set.seed(1)
vectmore <- rnorm(n = 1000000, mean = 10, sd = 5)
hist(vectmore, breaks=100)
...
```{r}
cdisfunc <- seq(-5,5, by= 0.1)
z=pnorm(cdisfunc)
plot = data.frame(x=cdisfunc, z)
ggplot(plot, aes(x=cdisfunc, y=z)) +
  geom_point(aes(x=cdisfunc, y=z))
...
```{r}
invdisfunc <- seq(0,1, by= 0.01)
invz=qnorm(invdisfunc)
plot = data.frame(x=invdisfunc, invz)
ggplot(plot, aes(x=invdisfunc, y=invz)) +
  geom_point(aes(x=invdisfunc, y=invz))

#qnorm used for inverse cdf
```

- Plot the density function in the range $[-5, 5]$ in 0.1 increment.

```{r}
denfunc <- seq(-5,5, by= 0.1)
denz=dnorm(denfunc)
plot = data.frame(x=denfunc, denz)
ggplot(plot, aes(x=denfunc, y=denz)) +
  geom_point(aes(x=denfunc, y=denz))
...
```{r}
pnorm(2) #probability of obtaining a number </= 2 in a normal distribution
probab2 <- 1-pnorm(2)
probab2
```
...
```{r}
value3 <- 1-(pnorm(2)-pnorm(-2)); value3
```
...
logcounts <- read_excel('/t1-data/project/obds/shared/resources/3_r_stats_genomics/statistics/data/GSE111543.xlsx', sheet=1)
dim(logcounts)
colnames(logcounts)
head(logcounts)
cell_info <- read_excel('/t1-data/project/obds/shared/resources/3_r_stats_genomics/statistics/data/GSE111543.xlsx', sheet=2)
dim(cell_info)
colnames(cell_info)
tail(cell_info)

library(tidyverse)
logcounts_pivot <- pivot_longer(logcounts, cols= starts_with('Cell'), names_to = 'Sample', values_to ='counts') 
head(logcounts_pivot)

logcounts_merge <- left_join(logcounts_pivot, cell_info, by= 'Sample')
head(logcounts_merge) #do a left join to keep all the rows in the logcounts_pivot
```
...
mock_condition <- cell_info$Sample[cell_info$Infection=='Mock']
head(mock_condition)
test_condition <- cell_info$Sample[cell_info$Infection!='Mock']
head(test_condition)

gene_1_mock <- logcounts_matrix[1,mock_condition]
gene_1_infected <- logcounts_matrix[1,test_condition]
head(gene_1_mock)

out_1stgene <- t.test(gene_1_mock, gene_1_infected)
out_1stgene
out_1stgene$p.value
logcounts_matrix
...
t_test_row <- function(index) {
  mock_condition <- cell_info$Sample[cell_info$Infection=='Mock']
test_condition <- cell_info$Sample[cell_info$Infection!='Mock']
gene_mock <- logcounts_matrix[index, mock_condition]
gene_test <- logcounts_matrix[index, test_condition]
  output <- t.test(gene_mock, gene_test)
  output$p.value}
t_test_row(index = 1)
```
...
```{r}
t_test_pvalues <- vapply( #apply on a vector
  X = 1:nrow(logcounts_matrix), #the function should be applied to rows 1 to last row in the matrix
  FUN = t_test_row,
  FUN.VALUE = 1) #1 here means that FUN.VALUE will be the same size as X
